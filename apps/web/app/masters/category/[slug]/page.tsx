...
export async function generateMetadata({
  ...
  const title = `${category.name} — мастера`;
  const description = `Частные мастера в разделе «${category.name}» в Ингушетии — отзывы, рейтинг, прямой контакт.`;
...
export default async function MasterCategoryPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <p className="text-sm font-medium text-muted-foreground">Мастера</p>
      ...
      <div className="mt-8">
        ...
        {items.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
            В этом разделе пока нет мастеров — станьте первым.
          </div>
        ) : (
