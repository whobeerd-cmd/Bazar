...
export default async function EditBusinessPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: business } = await supabase
    .from("businesses")
    .select(
      "id, owner_id, type, name, category_id, city_id, description, address_text, phone, whatsapp, instagram, website, hours, status, slug, specializations, price_from, experience_years, house_call"
    )
    .eq("id", id)
    .single();
    ...
    <div>
      <div className="flex items-center gap-3">
        <h1 className="text-2xl font-extrabold tracking-tight text-foreground">
          {isMaster ? "Редактирование анкеты мастера" : "Редактирование бизнеса"}
        </h1>
        <a
